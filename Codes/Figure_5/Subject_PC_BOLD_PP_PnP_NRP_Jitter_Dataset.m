%% Find PP and PnP Trials Across No Report Paradigm for Jitter ISI Trials
...
%main figure/PP/PnP matching ratio
%all_jitter_PP_num = 2062;
%all_jitter_PnP_num = 7935;
%main_fig_PP_num = 1785;
%main_fig_PnP_num = 1504;

PP_ratio = 0.6;%main_fig_PP_num/all_jitter_PP_num;
PnP_ratio = 0.45;%0.25;%main_fig_PnP_num/all_jitter_PnP_num;
...
for loc = 1:length(location_cell)
    ...
    %Loop over subject
    for sub = 1:length(noreport_subject_list) %[44,52,55]%42:43%1:length(noreport_subject_list) 
        ...
        elseif isequal(run_location, 's')  
            ...
            model_dir = fullfile('/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/Machine Learning Analysis/MRI Session'...
                ,current_location,'Base_boosted_tree_lin_gaus_SVM_stacked_lin_SVM_jitter_new');
        ...
        current_subject_opacity = subject_trial_opacity(find(strcmp(subject_trial_index,current_ID)));
        ...
        subject_confidence_scores_copy = subject_confidence_scores;
        subject_confidence_scores_copy(all_blink_at_stim_trials) = [];
        ...
        PP_idx = subject_confidence_scores_copy > confidence_score_threshold & current_subject_opacity_copy > 0 & current_subject_opacity_copy < 1;
        PnP_idx = subject_confidence_scores_copy < -confidence_score_threshold & current_subject_opacity_copy > 0 & current_subject_opacity_copy < 1;
        ...
        for run = 1:length(sub_runs)
            ...
            run_confidence_scores = subject_confidence_scores(extended_trial_idx);
            run_blink_matrix = blink_matrix(extended_trial_idx,:);
            run_opacity = subject_trial_opacity(extended_trial_idx);
            ...
            if not(isequal(size(run_confidence_scores,1), size(img_3D_PC_cut,3), size(times_img_3D_PC_cut,2),...
                    size(run_opacity,1),size(eval(['jitter_times_run_',num2str(current_run)]),1)))
            ...
            %Find predicted perceived and not perceived trials for specified SVM score threshold
            
            %Find PP and PnP Trials
            PP_idx = run_confidence_scores > confidence_score_threshold & run_opacity > 0 & run_opacity < 1;
            PnP_idx = run_confidence_scores < -confidence_score_threshold & run_opacity > 0 & run_opacity < 1;
            
            %Check if too many PP trials
            if [size(PP_BOLD,3) + sum(PP_idx)] > 125
            ...
            if [size(PnP_BOLD,3) + sum(PnP_idx)] > 125
        ...
        cd(event_dir) 
%        save(['PP_PnP_jitter_event_times_confidence_score_',num2str(confidence_score_threshold),'.mat'],'PP_event*','PnP_event*')
        ...
        %If matching trial number
        if isequal(match_sample_size,1)
            
            save(['extracted_signal_PC_predicted_jitter_matched_score_',num2str(confidence_score_threshold),'.mat'],...
               'PP_BOLD_subject_img_3D_PC_cut', 'PnP_BOLD_subject_img_3D_PC_cut', 'PP_trial_num', 'PnP_trial_num', 'PP_unfilled_num', 'PnP_unfilled_num', '-v7.3')
            
        else
            
            save(['extracted_signal_PC_predicted_jitter_score_',num2str(confidence_score_threshold),'.mat'], ...
