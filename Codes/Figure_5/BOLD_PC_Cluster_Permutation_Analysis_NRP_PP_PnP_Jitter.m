%% MR Voxel Based Permutation Analysis - PP and PnP Jitter
...
if isequal(data_type, 'subtract')
    ...
    %ROI_name = [ROI_string,'_PP_minus_PnP_',task_condition,'_',location_name,'_jitter_score_thres_',num2str(confidence_score),'_perm_',num2str(num_permutation)];
    ROI_name = [ROI_string,'_PP_minus_PnP_',task_condition,'_',location_name,'_jitter_matched_mod_score_thres_',num2str(confidence_score),'_perm_',num2str(num_permutation)];

else
    
    %ROI_name = [ROI_string,'_',data_type,'_',task_condition,'_',location_name,'_jitter_score_thres_',num2str(confidence_score),'_perm_',num2str(num_permutation)];
    ROI_name = [ROI_string,'_',data_type,'_',task_condition,'_',location_name,'_jitter_matched_score_thres_',num2str(confidence_score),'_perm_',num2str(num_permutation)];
...
load(['Group_PC_BOLD_',task_condition,'_PP_PnP_',location_name,'_jitter_matched_mod_score_thres_',num2str(confidence_score),'_data.mat'])
...
if isequal(data_type, 'subtract')
    ...
    elseif length(PP_epochs_subjects_list) > length(PnP_epochs_subjects_list)
        ...
        %Check if there is PnP subject but not in PP (edge case)
        if size(group_PnP_BOLD_PC_data,3) > size(group_PP_BOLD_PC_data,3)
                       
           group_PnP_BOLD_PC_data(:,:,not(ismember(PnP_epochs_subjects_list,tested_subjects_list))) = [];
