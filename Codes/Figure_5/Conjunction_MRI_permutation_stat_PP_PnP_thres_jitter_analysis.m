%% MRI Permutation Conjunction Analysis - PP vs PnP threshold and jitter

%Conjunction test: Threshold PP vs PnP and Jitter PP vs PnP
...
if comparison_direction == 1 
    ...
    if isequal(all_jitter_trials,1)
        ...
        comparison_folder_name = 'thres PP-PnP vs jitter PP-PnP';
    
    else
        
        comparison_folder_name = 'thres PP-PnP vs jitter matched PP-PnP';
...
elseif comparison_direction == 2 
    ...
    if isequal(all_jitter_trials,1)
        ...
        comparison_folder_name = 'jitter PP-PnP vs thres PP-PnP';

    else

        comparison_folder_name = 'jitter matched PP-PnP vs thres PP-PnP';
...
elseif isequal(run_location,'s')
    ...
    %CnP/Num matched jitter trials
    else
        
        %jitter_dir = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_PP_minus_PnP_rel_irrel_cent_quad_jitter_CnP_score_thres_',num2str(confidence_score),'_perm_100'];
        jitter_dir = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_PP_minus_PnP_rel_irrel_cent_quad_jitter_matched_score_thres_',num2str(confidence_score),'_perm_',num2str(perm_num)];

    end
    
    %PP minus PnP Threshold
    threshold_dir = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_PP_minus_PnP_irrelevant_cent_quad_threshold_score_thres_',num2str(confidence_score),'_perm_5000'];
    ...
    if isequal(comparison_direction, 1)
        ...
        if isequal(all_jitter_trials,1)
            ...
            thres_vs_jitter = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_thres_PP_minus_PnP_vs_jitter_PP_minus_PnP_rel_irrel_cent_quad_',num2str(confidence_score),'_perm_',num2str(perm_num)];
            
        else
        
            %thres_vs_jitter = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_thres_PP_minus_PnP_vs_jitter_CnP_PP_minus_PnP_rel_irrel_cent_quad_',num2str(confidence_score),'_perm_100'];
            thres_vs_jitter = ['/mnt/Data8/HNCT_NRP_Study/HNCT No Report Paradigm/Analysis/Group Analysis/MRI Analysis/Percent Change Permutation Stats/Whole Brain_thres_PP_minus_PnP_vs_jitter_matched_PP_minus_PnP_rel_irrel_cent_quad_',num2str(confidence_score),'_perm_',num2str(perm_num)];
...
if isequal(comparison_direction, 1)
    ...
    if isequal(non_sig_voxels_only, 1)
        ...
        ROI_name = ['Whole Brain ', comparison_folder_name, ' Score ', num2str(confidence_score), ' jitter_thres_not_sig_diff_voxels'];
...
jitter_sig_matrix = zeros(41,902629);

%Loop over postive clusters
for clust = 1:length(jitter_pos_clust)

    %Loop over time
    for time = 1:size(jitter_sig_matrix,1)
        ...
        jitter_sig_matrix(time,outInd(sig_grid_voxels)) = 1;

        %Give adjacent voxels to post clusters constant value
        jitter_sig_matrix(time,adj_voxels_array) = 1;      
...
%Threshold PP/PnP - Predicted Class
cd(threshold_dir)
load('pc_cluster_sumt_rand_005.mat', 'clust_info')

%Find significant clusters pvalue < 0.05 - Negative and positive clusters independently
thres_pos_clust = find(clust_info.pos_clust_pval < 0.05);
thres_neg_clust = find(clust_info.neg_clust_pval < 0.05);

%Create empty matrix channels by time
thres_sig_matrix = zeros(41,902629);

%Loop over postive clusters
for clust = 1:length(thres_pos_clust)

    %Loop over time
    for time = 1:size(thres_sig_matrix,1)

        %Find postive significant cluster number/idx in voxel x time matrix
        [sig_grid_voxels, col] = find(clust_info.pos_clust_ids(:,time) == thres_pos_clust(clust));
        ...
        %Give post clusters constant value - find the voxel all volume idx
        thres_sig_matrix(time,outInd(sig_grid_voxels)) = 1;

        %Give adjacent voxels to post clusters constant value
        thres_sig_matrix(time,adj_voxels_array) = 1;      
...
%Loop over negative clusters
for clust = 1:length(thres_neg_clust)

    %Loop over time
    for time = 1:size(thres_sig_matrix,1)

        %Find postive significant cluster number/idx in voxel x time matrix
        [sig_grid_voxels, col] = find(clust_info.neg_clust_ids(:,time) == thres_neg_clust(clust));
...
%Select only non-sig voxels from thres PP-PnP vs jitter PP-PnP stats
if isequal(non_sig_voxels_only,1)
...
additive_matrix = jitter_sig_matrix + thres_sig_matrix;
