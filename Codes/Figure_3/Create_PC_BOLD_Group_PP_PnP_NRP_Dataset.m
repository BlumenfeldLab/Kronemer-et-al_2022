%% Create Group Dataset of Percent Change BOLD Map - PP and PnP Trials Across No Report Paradigm
...
%Stimulus Opacity
prompt_5 = 'Stimulus opacity (opaque/threshold/blank): ';
stimulus_opacity = input(prompt_5, 's');
...
elseif isequal(relevant_condition, 'combine')
    ...
    elseif isequal(location_set, 'combine')
        ...
        location_cell = {'Center Relevant','Center Irrelevant','Quadrant Relevant','Quadrant Irrelevant'};
        save_loc_name = 'cent_quad';
...
for sub = 1:length(noreport_subject_list) 
    ...
    for loc = 1:length(location_cell)
        ...
        if ismember(current_location, {'Center Irrelevant','Quadrant Irrelevant'})
        ...
        elseif ismember(current_location, {'Center Relevant','Quadrant Relevant'})
        ...
        %Load PP/PnP BOLD
        cd(BOLD_dir)
        %load(['extracted_signal_PC_predicted_',stimulus_opacity,'_opacity_score_',num2str(confidence_score_threshold),'.mat'])
        load(['extracted_signal_PC_predicted_',stimulus_opacity,'_opacity_shifted_score_',num2str(confidence_score_threshold),'.mat'])
        
        %Check if PP BOLD data is empty before adding it to group
        if isempty(PP_BOLD_subject_img_3D_PC_cut) 
            
            disp(['PP BOLD data - not adding to group'])
        ...
        if isempty(PnP_BOLD_subject_img_3D_PC_cut) 
            ...
            disp(['PnP BOLD data - not adding to group'])
...
save(['Group_PC_BOLD_',condition_name,'_PP_PnP_',save_loc_name,'_',stimulus_opacity,'_score_thres_',num2str(confidence_score_threshold),'_data.mat'],...
