...
eeg_root = '\\blumenfeld1.med.yale.internal\Data7\RNS Study\Subject Raw Data\536JD\Scalp EEG Data\'; 
eeg_folders = {'Calibration and All Runs'};
eeg_files = {'Dibattiste~ Jo_53c440ca-b89e-43a7-9b00-1797d5710133-archive.lay'};
...
beh_root = {'\\blumenfeld1.med.yale.internal\Data7\RNS Study\Subject Raw Data\536JD\Behavioral Data'};
beh_folders = {'Calibration and All Runs'};
beh_files = {'536JD_Session 1_2019_Dec_03_1311_12.csv'};
...
for session = 1%:length(eeg_folders)
    ...
    save_dir = '\\blumenfeld1.med.yale.internal\Data7\RNS Study\Analysis\Subject Analysis\536JD\Scalp EEG Analysis';
...
%% Find the TTL pulses of interest and ID event onset times
    ...
    %Add TTL onset times (specific for 536JD***)
    onsets  = sort([onsets, 62770,612189,627646,631605,877549,880839,977711,1050523,1057039,1061269]);

    %Add TTL onset times (specific for 536JD***)
    offsets = sort([offsets, 62811,612228,627684,631643,877587,880877,977749,1050559,1057077,1061305]);
    
    %Remove TTL onset times that are not face events
    bad_TTL_pulse_idx = find(ismember(onsets,[118381,254118,390367,559851,689509,833861,962546,1094039]));
...
%     %Add pulses of interest manually (specific for 536JD***)
%     pulse_of_interest_onsets = [pulse_of_interest_onsets, find(ismember(onsets,[37549,877586,880839]))];
%     pulse_of_interest_onsets = sort(pulse_of_interest_onsets);
...
%     title('TTL face pulses (in red) during task')
    ...
    %Remove trial identifiers for missing files (Specific for 356JD*** Due
    %to missing face TTL pulses)
%     trial_identifiers([136,191,192,210]) = [];
