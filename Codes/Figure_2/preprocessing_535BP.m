...
eeg_root = '\\blumenfeld1.med.yale.internal\Data7\RNS Study\Subject Raw Data\535BP\Scalp EEG Data\'; 
eeg_folders = {'Calibration and All Runs'};
eeg_files = {'Platt~ Brittan_1cf4ef20-7444-452a-a506-b8fe7dd7f814-archive.lay'};
...
beh_root = {'\\blumenfeld1.med.yale.internal\Data7\RNS Study\Subject Raw Data\535BP\Behavioral Data'};
beh_folders = {'Calibration and All Runs'};
beh_files = {'535BP_Session 1_2019_Dec_03_1022_12.csv'};
...
session = 1; %:length(eeg_folders)
    ...
    save_dir = '\\blumenfeld1.med.yale.internal\Data7\RNS Study\Analysis\Subject Analysis\535BP\Scalp EEG Analysis';
...
%     save_dir = char(fullfile(save_dir, beh_folders(session)));
    ...
    %Remove data prior to run phases (selected by review data)
    record = record(:,1112408:2187886);
...
%% Find the TTL pulses of interest and ID event onset times
    ...
    bad_TTL_pulse_idx = find(ismember(onsets,[13721,13742,13751,14513,126566,126715,126724,126928,126941,127163,129484,...
        268859,407148,549994,550225,550336,550611,550653,550993,551472,552090,680831,681021,683096,683241,683663,821044,821168,...
        950629,950689,951008,951076,121477,257688,395761,541211,678349,805988,947801,1072375]));
