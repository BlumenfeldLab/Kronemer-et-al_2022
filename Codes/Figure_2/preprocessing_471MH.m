...
eeg_root = 'R:\RNS Study\Subject Raw Data\471MH\Scalp EEG Data';
eeg_folders = {'Calibration and All Runs'};
eeg_files = {'Hearn~ Test_6ee4f9d1-007d-48d8-b52a-d6f5608e8048-archive.lay'};

%% Location of behavior folder/files

beh_root = {'R:\RNS Study\Subject Raw Data\471MH\Behavioral Data'};
beh_folders = {'Calibration and All Runs'};
beh_files = {'471MH_Session_1_2019_Mar_29_1123_19.csv'};
...
%Loop over sessions
session = 1;%:length(eeg_folders)

% Location to store epochs
save_dir = 'R:\RNS Study\Analysis\Subject Analysis\471MH\Scalp EEG Analysis';
...
EEG = record;
ID = '471MH';
cd('R:\RNS Study\Analysis\Analysis Code\Preprocessing\Preprocessing Pipeline Functions')
EEG_highpass_cleanline = eeg_session_level_processing_RNS(EEG, ID, save_dir, cleanline_dir, run_location, sampling_rate);
...
%Remove non-face pulses of interest (one at the end of each trial - specific for 471MH***)
pulse_of_interest_onsets([18,37,55,74,93,112,131,151,170,189,209,228,249,269,288]) = [];

%Plot TTL channel and the points marking event onset
figure;
...
for type = 1%:length(epochtypes)
    ...
    %Save epochs
    cd(save_dir)
    save('avg_ref_face_2000ms_epochs.mat','epochs','header','-v7.3');
    ...
    if type == 1
        ...
        %Save epochs
        cd(save_dir)
        save('avg_ref_face_6000ms_epochs.mat','epochs','header','-v7.3');
...
save('ttl_times_and_identity.mat','pulse_of_interest_sample_idx', 'trial_identifiers', '-v7.3')
 ...
 %% epoch level rejections 
    ...
    CP_all_epochs_reref_interp = epoch_level_bad_channel_removal_RNS(CP_epochs, 'CP',save_dir, voltage_threshold, high_volt_prop, EEG_highpass_cleanline, prop_bad_channels);
    cd('V:\RNS Study\Analysis\Analysis Code\Preprocessing\Preprocessing Pipeline Functions')
    CnP_all_epochs_reref_interp = epoch_level_bad_channel_removal_RNS(CnP_epochs, 'CnP',save_dir, voltage_threshold, high_volt_prop, EEG_highpass_cleanline, prop_bad_channels);
    
    cd(strcat('\\blumenfeld1.med.yale.internal\data7\RNS Study\Analysis\Subject Analysis\',ID,'\Scalp EEG Analysis'))
    save post_rej_epochs CP_all_epochs_reref_interp CnP_all_epochs_reref_interp
